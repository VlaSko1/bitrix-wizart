		...
		public function Validate() {
          ...
          $rule_value = '';
					...
					switch (strtoupper(trim($rule))) {
						...
						case "REQ" :
						case "REQUIRED" : {
							if (! array_key_exists($key, $this->Vars)) return false;
							break;
						}
						...
						case "MIN_LENGTH" : {
							if (isset($this->Vars[$key])) {
								if (mb_strlen($this->Vars[$key]) < $rule_value) return false;
							}
							break;
						}
            ...
            case "REGULAR" : {
              if (isset($this->Vars[$key])) {
                if (! preg_match($rule_value, $this->Vars[$key])) return false;
              }
              break;
            }
            ...
            case "FILE" : {
              if (isset($this->Vars[$key])) {
                if (explode('/', mime_content_type($this->Vars[$key]['tmp_name']))[0] !== 'image') return false;
                if (! preg_match('/^(png|jpg|jpeg|bmp)$/', explode('/', mime_content_type($this->Vars[$key]['tmp_name']))[1])) return false;
                if ($this->Vars[$key]['size'] < 40 || $this->Vars[$key]['size'] > 900000) return false;
              }
              break;
            }
					}
